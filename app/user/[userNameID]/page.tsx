                    ...
                    {!files ? (
                        // Private Profile View
                         <div className="text-center py-20 bg-zinc-900/30 rounded-3xl border border-zinc-800/50">
                            ...
                            <h3 className="text-xl font-medium text-zinc-300">Este perfil es privado</h3>
                            <p className="text-zinc-500 mt-2">El usuario ha configurado su perfil como privado. <br/> No puedes ver sus archivos.</p>
                        </div>
                    ) : files.length === 0 ? (
                        // Public but empty
                        <div className="text-center py-20 bg-zinc-900/30 rounded-3xl border border-zinc-800/50">
                            <FileIcon className="w-16 h-16 text-zinc-700 mx-auto mb-4" />
                            <h3 className="text-xl font-medium text-zinc-300">Sin archivos públicos</h3>
                            ...
                            {files.map((file: any) => (
                                ...
                                >
                                    ...
                                    <div className="flex items-start gap-4 mb-4">
                                        <div className="p-3 bg-zinc-800 rounded-xl group-hover:bg-zinc-800/80 transition-colors">
                                            <FileIcon className="w-8 h-8 text-primary" />
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <h3 className="font-medium text-lg leading-snug truncate pr-6" title={file.originalName}>
                                                {file.originalName}
                                            </h3>
                                            <div className="flex items-center gap-2 mt-1 text-xs text-zinc-500">
                                                <span className="bg-zinc-800 px-2 py-0.5 rounded text-zinc-400">{file.mimeType.split('/')[1]?.toUpperCase() || 'FILE'}</span>
                                                <span>•</span>
                                    ...
                                    <div className="flex items-center justify-between text-xs text-zinc-500 pt-4 border-t border-zinc-800/50 group-hover:border-primary/20 transition-colors">
